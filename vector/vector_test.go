...
func TestAggregateEmbeddings_NegativeWeights(t *testing.T) {
	...
	weights := []float32{-0.5, 1.5}

	result := AggregateEmbeddings(vectors, weights)

	assert.Len(t, result, 2)
	// Expected: (1*-0.5 + 3*1.5) = -0.5 + 4.5 = 4.0
	assert.InDelta(t, float32(4.0), result[0], 0.01)
	// Expected: (2*-0.5 + 4*1.5) = -1.0 + 6.0 = 5.0
	assert.InDelta(t, float32(5.0), result[1], 0.01)
}
