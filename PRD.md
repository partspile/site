...
- Each ad is encoded as a vector embedding at creation or update, stored in Pinecone with metadata (created_at, click_count, parent_company, parent_company_country, etc.).
...
- All search and feed functionality is powered by Pinecone vector search only.
