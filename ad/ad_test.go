...
func TestPopulateImageOrderSlice(t *testing.T) {
	tests := []struct {
		name           string
		imageOrder     string
		expectedLength int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ad := Ad{
				ImageOrder: tt.imageOrder,
			}

			ad.PopulateImageOrderSlice()

			assert.Equal(t, tt.expectedLength, len(ad.ImageOrderSlice))
			if tt.expectedLength > 0 {
				assert.Equal(t, tt.expectedFirst, ad.ImageOrderSlice[0])
			}
		})
	}
}
